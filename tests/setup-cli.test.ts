...
test('local onboarding creates an ignored private credential file without echoing or replacing credentials',()=>{
 ...
 try{
  copyFileSync('.env.example',join(cwd,'.env.example'));copyFileSync('relationships.example.json',join(cwd,'relationships.example.json'));
  ...
  const relationships=readFileSync(join(cwd,'private/relationships.json'),'utf8');assert.equal(JSON.parse(relationships).version,1);if(process.platform!=='win32')assert.equal(statSync(join(cwd,'private/relationships.json')).mode&0o777,0o600);
  const second=run();assert.equal(readFileSync(join(cwd,'private/relationships.json'),'utf8'),relationships);assert.equal(second.status,0,second.stderr);assert.equal(readFileSync(join(cwd,'.env'),'utf8'),saved);assert.ok(!second.stdout.includes(token));
