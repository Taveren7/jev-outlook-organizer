...
try{
 ...
 const message:any={id:'custom-test','@odata.etag':'v1',receivedDateTime,parentFolderId:'inbox',categories:['Keep Me'],flag:{flagStatus:'notFlagged'},isRead:false,subject:'Synthetic milestone',from:{emailAddress:{address:'sender@example.test'}},toRecipients:[],ccRecipients:[],body:{contentType:'text',content:'Please respond immediately about the delivery milestone.'},hasAttachments:true};
 ...
 assert.equal(modelCalls,1);assert.equal(message.parentFolderId,'projects-folder');assert.deepEqual(message.categories,['Keep Me','Urgent','Respond','Check This','My Follow-up']);assert.equal(message.isRead,false);assert.deepEqual(message.flag,{flagStatus:'notFlagged'});assert.deepEqual(effects,['categories','move']);
