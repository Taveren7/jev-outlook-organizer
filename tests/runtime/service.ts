...
let message:any={id:'test-message','@odata.etag':'v1',parentFolderId:'inbox',categories:['Personal'],isRead:false,flag:{flagStatus:'notFlagged'},receivedDateTime,subject:'synthetic',from:{emailAddress:{address:'sample@example.test'}},toRecipients:[],ccRecipients:[],body:{contentType:'text',content:'synthetic receipt'},hasAttachments:false};
...
try{
 ...
 assert.equal(writes,3);assert.deepEqual(message.categories,['Personal','FYI','Reference']);assert.equal(message.isRead,false);assert.equal(message.parentFolderId,'pack_invoice_confirmation');
